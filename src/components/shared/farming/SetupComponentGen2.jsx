...
const SetupComponentGen2 = (props) => {
    ...
    useEffect(async () => {
        try {
            ...
            if(ethIndex !== -1) {
                var ethPrice = await window.getEthereumPrice();
    ...
    const removeLiquidity = async () => {
        ...
        try {
            ...
            try {
                await submit(lmContract.methods.withdrawLiquidity(currentPosition.positionId, removedLiquidity, amMin[0].full || amMin[0], amMin[1].full || amMin[1], "0x"));
            } catch(ex) {
                if((ex.message || ex).toLowerCase().indexOf('user denied') === -1) {
                    await submit(lmContract.methods.withdrawLiquidity(currentPosition.positionId, removedLiquidity, "0x"));
                }
            }
            await getSetupMetadata();
        } catch (error) {
    ...
    async function withdrawAll() {
        ...
        try {
            ...
            try {
                await submit(lmContract.methods.withdrawLiquidity(currentPosition.positionId, manageStatus.liquidityPoolAmount, amMin[0].full || amMin[0], amMin[1].full || amMin[1], "0x"));
            } catch(ex) {
                if((ex.message || ex).toLowerCase().indexOf('user denied') === -1) {
                    await submit(lmContract.methods.withdrawLiquidity(currentPosition.positionId, manageStatus.liquidityPoolAmount, "0x"));
                }
            }
            await getSetupMetadata();
        } catch(e) {
